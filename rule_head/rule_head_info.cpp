                      ...
                      )
{
    ...
    if (pos != p.nt_table.end()) {
        pdist = &(pos->second);
        bpdist = &(p.predicted_table.find(nt)->second);
    }
    else {
        pdist = &p.word_unigram;
        bpdist = &p.word_unigram;
    }
    ...
    double retval = wb_prob(c,n,prob,wb_prob(bc,bn,bprob,bbprob,dmult),dmult);
    if (retval == 0.0)
                           ...
                           )
{
    ...
    double retval = wb_prob(c,n,prob,bkf,dmult);
    if (retval == 0.0)
                            ...
                            , word_t headword )
{
    ...
    dist const& bdist = pos->second.get<1>().at(v);
    double primary = 0.0;
    word_t h;
    double p;
    BOOST_FOREACH(dist_entry_t const& bd, bdist.distribution) {
        boost::tie(h,p) = bd;
        word_table_t::const_iterator tpos = p.transmissions.find(h);
        if (tpos == p.transmissions.end()) throw std::runtime_error("rule-head: incomplete transmissions table");
        double trans = get_(tpos->second.distribution,headword).get<1>();
        primary += p * trans;
    }
    count_t bn = p.word_unigram.n;
    count_t bc = p.word_unigram.c;
    return wb_prob(bn,bc,primary,1.0/double(bc),dmult);
}
...
std::pair<word_t,dist> read_transmission_table_line(std::string const& line)
{
    ...
    while (sstr >> word) {
        double p;
        sstr >> p;
        entries.push_back(dist_entry_t(get_index(word),p));
    }
    return std::make_pair(given,dist(0,0,entries,false));
}

word_table_t& read_transmission_table(istream& in, word_table_t& tbl)
{
    tbl.clear();
    std::string line;
