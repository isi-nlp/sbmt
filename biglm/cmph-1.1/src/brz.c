...
static int brz_gen_mphf(cmph_config_t *mph)
{
	...
	// convert to float for compatibility with 0.6 (DC)
	float c = brz->c;
	nbytes = fwrite(&c, sizeof(c), (size_t)1, brz->mphf_fd);
...
void brz_load(FILE *f, cmph_t *mphf)
{
	...
	// convert from float for compatibility with 0.6 (DC)
	float c;
	nbytes = fread(&c, sizeof(c), (size_t)1, f);
	brz->c = c;
