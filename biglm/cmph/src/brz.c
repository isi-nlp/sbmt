...
void brz_load(FILE *f, cmph_t *mphf)
{
	...
	for(i = 0; i < brz->k; i++)
	{
        ...
        /* cache some values. DWC*/
        if (brz->algo == CMPH_FCH) {
          for (i=0; i<256; i++) {
            brz->fch_b[i] = fch_calc_b(brz->c, i);
	    brz->fch_p1[i] = fch_calc_p1(i);
            brz->fch_p2[i] = fch_calc_p2(brz->fch_b[i]);
	  }
	}
...
static cmph_uint32 brz_fch_search(brz_data_t *brz, const char *key, cmph_uint32 keylen)
{
	cmph_uint32 h0 = hash(brz->h0, key, keylen) % brz->k;
	cmph_uint32 m = brz->size[h0];
	cmph_uint32 b = brz->fch_b[m];    /*fch_calc_b(brz->c, m);*/
	cmph_float32 p1 = brz->fch_p1[m]; /*fch_calc_p1(m);*/
	cmph_float32 p2 = brz->fch_p2[m]; /*fch_calc_p2(b);*/
