...
#endif
	typename hash_map<string, ORNode<T>*> :: const_iterator it;
	for(it = m_indexBySig.begin(); it != m_indexBySig.end(); ++it){
...
private:
	...
	//! reading.
	ORNode<T>* read_an_OR_node(istream& in, int height);
	ANDNode<T>* read_an_AND_node(istream& in, int height);
...
private:
	...
	void cutLeavesOfSubForest(mtrule::TreeNode*, 
