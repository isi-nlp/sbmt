...
template<class T> 
ORNode<T>*PackedForest<T>::read_an_OR_node(istream& in)
{
	char ch;
	while(in.peek() == ' '){ in>>ws;}
	...
	if(in.peek() == ')'){
		...
		// form3  (#id leaf)
		while(in.peek() != ')'){
			ANDNode<T>* pandNode=read_an_AND_node(in);
...
// (label #1(children)).  
template<class T> 
ANDNode<T>*PackedForest<T>::read_an_AND_node(istream& in)
{
	char ch;
	...
	if(in.peek() == '('){
		...
		while(in.peek() != ')'){
			try {
				andNode->push_back(read_an_OR_node(in));
			} catch (ForestFormatError& e){
				throw e;
				return NULL;
			}
...
void PackedForest<T>::cutLeaves()
{
	...
	cutLeavesOfSubForest(getRoot(), nodes);
...
template<class T>
void PackedForest<T>::cutLeavesOfSubForest(mtrule::TreeNode* node, set<mtrule::TreeNode*>& nodes)
{
	if(node) {
