...
inline void throw_string_to(std::string content,std::string const& msg,char const* prefix="string_to: ") NORETURN;
inline void throw_string_to(std::string content,std::string const& msg,char const* prefix) {
  if (content.empty())
    content=prefix;
  else
    content=prefix+std::string("'")+content+"' ";
  throw std::runtime_error(content+msg);
}
...
inline void string_to_impl(std::string const& s,bool &x) {
  if (s==str_1||s==str_true||s==str_yes||s==str_on)
    ...
    throw_string_to(s,"not boolean ({true,1,yes,on} or {false,0,no,off}.");
}

inline void string_to_impl(std::string const& s,char &x) {
  if (s.size()!=1)
    throw_string_to(s,"converting string to character.");
  x=s[0];
}
...
void string_to_impl(From const& from,To &to)
{
  ...
  if (!try_string_to(from,to))
    throw std::runtime_error(std::string("Couldn't convert (string_to): ")+str);
#endif
}
...
struct to_string_select<V,typename boost::enable_if<is_pair<V> >::type> {
  ...
  static inline void string_to(Str const &s,V &v)
  {
    ...
    if (p==string::npos)
      throw_string_to(s,"pair missing "+string_to_sep_pair+" separator");
