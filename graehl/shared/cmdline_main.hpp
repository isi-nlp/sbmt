...
struct main {
  ...
  {
#if !ZGRAEHL_CMDLINE_MAIN_USE_CONFIGURE
    SHOWIF1(CONFEXPR,1,"add_options_base",this);
    if (bopt.add_help)
...
public:
  ...
  template <class Val>
  void configurable(Val *pval,std::string const& groupname)
  {
    configurable(pval,configure::opt_path(1,groupname));
  }
  ...
  bool parse_args(int argc, char **argv)
  {
    ...
    try {
      ...
      if (exec->maybe_po_help(std::cout))
        return false;
      confs.store(to_cerr);
#else
