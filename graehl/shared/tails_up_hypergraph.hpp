...
struct BestTreeOptions
{
  ...
  template <class Conf> void configure(Conf &c)
  {
    ...
    c("convergence",&convergence_epsilon_str)("if empty or 0, only stop on maximum # of rereaches or 0 change anywhere. else stop if change is below this amount").is("epsilon (nonnegative real)");
  }
