# to do:
#  - some references to sym still
#  - purge unused file formats

import sys, math
import itertools, heapq, collections, random
import re, xml.sax.saxutils
import rule, cost, svector, log
import tree
...
class Item(object):
    ...
    def __str__(self):
        if self.x is None:
            ...
            return "[%s,%d,%d,%s,cost=%s]" % (self.x,self.i,self.j,str(self.states),self.viterbi)
...
def _ded_to_text(node, result, memo, mode=None, weights=None):
    ...
    if False and len(node.ants) == 0: # the format allows this but only if we don't tag with an id. but we tag everything with an id
        ...
        for child in children:
            ...
            elif isinstance(child, rule.Nonterminal):
                result.append(' ')
                _item_to_text(node.ants[child.getindex()-1], result, memo, mode=mode, weights=weights)
            else:
                result.append(' ')
                result.append(quoteattr(str(child)))
        result.append(')')

class TreeFormatException(Exception):
    pass

dummylabel = rule.Nonterminal("X")
dummyi = dummyj = None
