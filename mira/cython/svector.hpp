...
protected:
  ...
  struct and_op {
    svector<F,V> result;
    void operator()(F f, V x, V y) { result[f] = 1.; }
  };
...
public:
  ...
  svector operator&(const svector<F,V>& y) const { // deprecated, I don't know if this was a good idea
    and_op op;
    intersect(y, op);
    return op.result;
  }
  ...
  void compact(void) {
    iterator it = m.begin(), next;
    while (it != m.end()) {
      if (std::fabs(it->second) == 0.0) {
	m.erase(it++);
      } else {
	++it;
      }
