...
cdef extern from "svector.hpp":
    ctypedef struct c_svector "svector<int,double>":
        ...
        c_svector bitwise_and "operator&" (c_svector)
...
cdef class Vector:
    ...
    def normsquared(Vector self):
        return self.thisptr.normsquared()

    def compact(Vector self):
        self.thisptr.compact()

    def __and__(Vector self, Vector other):
        cdef Vector z = Vector()
        del_svector(z.thisptr)
        z.thisptr = copy_svector(self.thisptr.bitwise_and(other.thisptr[0]))
        #self.thisptr.bitwise_and(other.thisptr[0]).swap(z.thisptr[0])
        return z
