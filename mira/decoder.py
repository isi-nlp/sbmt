...
import forest, model, sym, rule, lattice, svector, cost
import log, monitor
    ...
    def read(self, f, models, weights):
        if type(f) is str:
            if os.path.isfile(f):
                ...
                raise IOError("Could not load grammar")
        else:
            if log.level >= 1:
...
def get_nbest(goal, n_best, ambiguity_limit=None, n_random=None):
    ...
    if n_random:
        insides = goal.compute_inside(thedecoder.weights, beta=0.)
...
if __name__ == "__main__":
    ...
    optparser.add_option("--output-alignments", dest="alignment_file")
    ...
    if opts.alignment_file:
        alignment_file = open(makefilename(opts.alignment_file), "w")
    else:
        alignment_file = None
    ...
    def process(sent):
        ...
        outputs = get_nbest(goal, n_best, n_random=n_random, ambiguity_limit=ambiguity_limit)
        ...
        if alignment_file:
            alignment_file.write("%s ||| %s\n" % (sent.id, ' '.join('%s-%s' % (fi, ei) for (fi, ei) in goal.viterbi_deriv().alignment())))
