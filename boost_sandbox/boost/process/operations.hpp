...
{ 
    ...
    { 
        ...
        if (!envpath) 
            boost::throw_exception(std::runtime_error(std::string("boost::process::find_executable_in_path: retrieving PATH failed: ") + file)); 
    ...
    if (result.empty()) 
        boost::throw_exception(std::runtime_error(std::string("boost::process::find_executable_in_path: file not found: ") + file)); 
