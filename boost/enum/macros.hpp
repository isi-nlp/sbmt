...
#define BOOST_ENUM_names_assert(index,size) \
  index_type idx = index; \
  BOOST_ASSERT(static_cast<index_type>(idx) < size)
#else
#define BOOST_ENUM_names_assert(index,size) (void)0
#endif 

#define BOOST_ENUM_names(_seq, _col, _colsize) \
	static const char* names(domain index) \
	{ \
	        BOOST_ENUM_names_assert(index,size);	\
		switch(index) \
		{ \
		BOOST_PP_CAT(BOOST_ENUM_VISITOR, _colsize) \
