...
public:
    ...
    typedef typename info_type::lm_id_type lm_id_type;
    typedef LM lm_type;
...
struct lm_phrase
{
    ...
    info_accessors<N> nia;
    nntm_factory<N> const* factory;
    ...
    , nia(factory->start,factory->end,factory->null)
    {
        ...
        ngram = new boost::uint32_t[N + factory->source_window];
        reset();
    }
 ...
 private:
    ...
    sbmt::score_t prob(lm_id_type const* beg, lm_id_type const* end)
    {
        ...
        int spn = word.srcid;
        int m = factory->source_window / 2;
        nntm_pathmap::const_iterator pos = factory->pmap.find(spn);

        for (; m != factory->source_window; ++m) {
        ...
        BOOST_FOREACH(int nwd, std::make_pair(ngram,ngram + m)) {
            if (count == factory->source_window) std::cerr << " ###";
            ++count;
    ...
    void finish_right(info_type &result)
    {
        ...
        typename info_type::iterator rb = nia.right_begin_full(result),
                                     r,
...
template <int N>
std::string
nntm_factory<N>::hash_string( sbmt::in_memory_dictionary const& dict
                            , typename info_type::lm_id_type const& info ) const
{
                         ...
                         , info_type const& info ) const
{
    ...
    for (int x = 0; x != 2; ++x) {
        if (x != 0) sstr << "//";
        for (int y = 0; y != info_type::ctx_len; ++y) {
            if (y != 0) sstr << "_";
...
template <int N>
nntm_factory<N>::nntm_factory( sbmt::lattice_tree const& ltree
...
, source_window(source_window)
{
    ...
    start = typename info_type::lm_id_type(model->lookup_input_word("<s>"),model->lookup_output_word("<s>"),0);
    null  = typename info_type::lm_id_type(model->lookup_input_word("<null>"),model->lookup_output_word("<null>"),0);
    int index;
    ...
    end = typename info_type::lm_id_type(model->lookup_input_word("</s>"),model->lookup_output_word("</s>"),index);
...
template <int N>
void nntm_factory<N>::init( sbmt::lattice_tree::node const& lnode
                          , std::map<sbmt::indexed_token,int> const& inputmap
                          , std::map<sbmt::indexed_token,int> const& outputmap
                          , sbmt::in_memory_dictionary& dict )
{
	...
	std::cerr << "NNTM backoff tag: " << dict.label(lnode.lat_edge().source) << " -> " << dict.label(tag) << "\n";
...
template <int N>
nntm_factory<N>::nntm_factory(nntm_factory const& o)
: pmap(o.pmap)
, revpmap(o.revpmap)
, lm(o.lm)
, lmstrid(o.lmstrid)
...
, unkid(o.unkid)
, outunkid(o.outunkid)
, source_window(o.source_window)
{}

template <int N>
void
nntm_factory<N>::compute_ngrams( info_type &n
                               , sbmt::score_t &inside
                               ...
                               , kids_type const& lmk )
{
    ...
    if (not phrase.get()) phrase.reset(new lm_phrase<N>(256,this));
    phrase->reset();
    if (is_toplevel) phrase->start_sentence();

    BOOST_FOREACH(sbmt::lm_token<typename info_type::lm_id_type> const& i, lmstr) {               
        if(i.is_token()){
...
template <int N>
bool nntm_factory<N>::start_symbol(kids_type const& lmk, affiliation_vector const& lmstr) const
{
...
template <int N>
bool nntm_factory<N>::deterministic() const { return true; }
...
sbmt::options_map nntm_factory_constructor::get_options() 
{
    ...
    opts.add_option( "nntm-target-order", optvar(target_ngram),"");
    opts.add_option( "nntm-source-window", optvar(source_window),""); 
    return opts;
}
