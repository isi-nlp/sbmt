...
struct lm_phrase
{
 ...
 private:
    ...
    sbmt::score_t prob(lm_id_type const* beg, lm_id_type const* end)
    {
        ...
        int spn = word.get<1>();
        int m = F / 2;
        pathmap::const_iterator pos = factory->pmap.find(spn);

        for (; m != F; ++m) {
            if (pos != factory->pmap.end()) {
                ngram[m] = factory->lm_id(pos->second.begin()->first);
                pos = factory->pmap.find(pos->second.begin()->second);
            } else {
        ...
        for (; m != -1; --m) {
            if (pos != factory->revpmap.end()) {
                ngram[m] = factory->lm_id(pos->second.begin()->first);
                pos = factory->revpmap.find(pos->second.begin()->second);
            } else {
        ...
        for (int f = 0; f < N - thislen; ++f, ++m) {
            ngram[m] = factory->start.get<0>();
        } 
        for (; itr != end - 1; ++itr, ++m) {
            ngram[m] = itr->get<0>();
        }
        // affiliation of current - affiliation of previous
        int curraf = itr->get<1>();
        int prevaf = (itr - 1)->get<1>();
        int distortion = std::min(std::max(curraf - prevaf, -6),6);
        ngram[m] = factory->lm_outid(distortion);
        # if PRINT_NNDM
        std::cerr << "\nNNDM: SCORE:";
        int count = 0;
        ...
        return score(factory->lm->lookup_ngram(ngram,m+1));
        //ngram
    }
...
std::string
nndm_factory::hash_string( sbmt::in_memory_dictionary const& dict
                         , info_type::lm_id_type const& info ) const
{
    std::stringstream sstr;
    sstr << '"' << lm->get_input_vocabulary().words()[info.get<0>()] << '"';
    if (info != null) {
        assert (pmap.find(info.get<1>()) != pmap.end());
        sstr << ',' << '"' << dict.label(pmap.find(info.get<1>())->second.begin()->first) << '"';
    }
    return sstr.str();
}
...
nndm_factory::nndm_factory( sbmt::lattice_tree const& ltree
...
, outputmap(&outputmap)
{
    ...
    BOOST_FOREACH(boost::tie(fs,toindex),pmap.begin()->second) {
        if (index != 0 or fs != dict.foreign_word("<foreign-sentence>")) throw std::runtime_error("in nndm: expected <foreign-sentence>,0");
    }
    index = pmap.rbegin()->first;
    BOOST_FOREACH(boost::tie(fs,toindex),pmap.rbegin()->second) {
        if (fs != dict.foreign_word("</foreign-sentence>")) throw std::runtime_error("in nndm: expected </foreign-sentence>,-1");
    }
...
void nndm_factory::init( sbmt::lattice_tree::node const& lnode )
{
...
nndm_factory::nndm_factory(nndm_factory const& o)
: pmap(o.pmap)
, revpmap(o.revpmap)
, lm(o.lm)
, lmstrid(o.lmstrid)
...
void
nndm_factory::compute_ngrams( info_type &n
                            , sbmt::score_t &inside
...
void nndm_factory_constructor::init(sbmt::in_memory_dictionary& dict)
{
    using namespace sbmt;
    if (filename != "") {
        if (not model) {
            std::cerr << "loading nndm model";
        ...
        output_map.insert(std::make_pair(6,model->lookup_output_word(">+5")));
        output_map.insert(std::make_pair(-6,model->lookup_output_word("<-5")));
        output_map.insert(std::make_pair(0,model->lookup_output_word("+0")));
...
bool nndm_factory::start_symbol(kids_type const& lmk, affiliation_vector const& lmstr) const
{
    for (size_t x = 0; x != lmk.size(); ++x) {
        if (sbmt::is_nonterminal(lmk[x].root())) { 
            if ((*lmk[x].info())(0,0).get<0>() == lm->start) {
                return true;
            }
        }
    }
    if (lmstr.begin() != lmstr.end() and lmstr.begin()->is_token() and lmstr.begin()->get_token().get<0>() == lm->start) return true;
    return false;
}

bool nndm_factory::deterministic() const { return true; }

bool nndm_factory_constructor::set_option(std::string name, std::string value)
{
    return false;
}

sbmt::options_map nndm_factory_constructor::get_options() 
{
    using namespace sbmt;
    options_map opts("nndm model options");
    opts.add_option( "nndm-model-file", optvar(filename),"nndm file");
    return opts;
}
