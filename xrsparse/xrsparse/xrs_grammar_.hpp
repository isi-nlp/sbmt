...
//# define BOOST_SPIRIT_SINGLE_GRAMMAR_INSTANCE 1
...
//# ifndef NDEBUG
//# define DISABLE_SPIRIT_DEBUG
//# define NDEBUG 1 // dont let spirit write debug code... bugs.
//# endif
# include <boost/spirit/include/classic.hpp>
//# ifdef DISABLE_SPIRIT_DEBUG
//# undef NDEBUG
//# endif
...
template <class T>
struct closure : boost::spirit::classic::closure<closure<T>,T> {
...
class brf_grammar
  : public boost::spirit::classic::grammar<brf_grammar> {
public:
    ...
    struct definition {
        ...
        boost::spirit::classic::rule<S> rhs_item;
        ...
        definition(brf_grammar const& g)
        {
            ...
            word = (ch_p('"') >> ch_p('"') >> ch_p('"'))[assign_(word_,"\"")]
	         | (ch_p('"') >> ch_p('@') >> ch_p('"') >> ch_p('"'))[assign_(word_,"@\"")]
                 | (ch_p('"') >> ch_p('"') >> ch_p('@') >> ch_p('"'))[assign_(word_,"\"@")]
	         | (ch_p('"') >> ch_p('@') >> ch_p('"') >> ch_p('@') >> ch_p('"'))[assign_(word_,"@\"@")]
...
class xrs_grammar
  : public boost::spirit::classic::grammar<xrs_grammar> {
public:
    rule_data& value;
    xrs_grammar(rule_data& value) : value(value) {}
    template <class S>
    struct definition {
        typedef typename S::iterator_t iterator_t;
        typedef boost::iterator_range<iterator_t> range_t;
        ...
        boost::spirit::classic::subrule<0> feats;

        fpair_t feat_;
        boost::spirit::classic::subrule<1> feat;
        range_t fname_;
        boost::spirit::classic::subrule<2> fname;
        fvalue_t fvalue_;
        boost::spirit::classic::subrule<3> fvalue;
        range_t simple_str_;
        boost::spirit::classic::subrule<4> simple_str;
        range_t compound_str_;
        boost::spirit::classic::subrule<5> compound_str;
        double real_;
        double score_;
        boost::spirit::classic::subrule<6> score;

        boost::spirit::classic::subrule<0> rhs_items;
        rhs_node rhs_item_;
        boost::spirit::classic::subrule<1> rhs_item;
        size_t rhs_index_;
        boost::spirit::classic::subrule<2> rhs_index;
        range_t rhs_word_;
        boost::spirit::classic::subrule<3> rhs_word;

        lhs_node lhs_node_;
        size_t lhs_root_;
        std::vector<size_t> lhs_root_stack_;
        boost::spirit::classic::subrule<0> lhs_root;
        std::vector<size_t> lhs_seq_;
        boost::spirit::classic::subrule<1> lhs_seq;
        size_t lhs_seq_item_;
        boost::spirit::classic::subrule<2> lhs_seq_item;
        range_t lhs_word_;
        boost::spirit::classic::subrule<3> lhs_word;
        size_t lhs_index_;
        boost::spirit::classic::subrule<4> lhs_index;
        range_t nt_;
        boost::spirit::classic::subrule<5> nt;
        pair_t indexed_nt_;
        boost::spirit::classic::subrule<6> indexed_nt;
        
        boost::spirit::classic::int_parser<rule_data::rule_id_type, 10, 1, -1> id_p;
        ...
        definition(xrs_grammar const& g)
        {
            ...
            features =
             ( feats = *( lexeme_d["id=" >> id_p[assign_(g.value.id)]]
                        | feat[ push_back_a(g.value.features,feat_) ]
                        )
             , feat = lexeme_d[
                         fname[ assign_(feat_.first,fname_) ]
...
rule_data parse_xrs(Range const& line)
{
    ...
    if (not info.full) {
        ...
        sstr << "failed to parse "
             << std::string(boost::begin(line),boost::end(line)) 
             << ": " 
             << std::string(info.stop,end(line));
    ...
    if (ridx != lidx) {
        std::stringstream sstr;
        sstr << "rhs and lhs had different numbers of variables, line "
        << std::string(boost::begin(line),boost::end(line));
        throw std::runtime_error(sstr.str());
    }
    assert(ridx == lidx);
...
brf_data parse_brf(Range const& line)
{
    ...
    if (not info.full) {
        std::stringstream sstr;
        sstr << "failed to parse "<< std::string(boost::begin(line),boost::end(line)) << ": " << std::string(info.stop,end(line));
        throw std::runtime_error(sstr.str());
    }
    return brf_g.value;
}
